...
class DotSeparator extends React.Component {
  removeEmptyElement(actualArray) {
    ...
    while (++index < arr_length) {
      var value = actualArray[index]
        ? actualArray[index].toString().trim()
        : '';
      if (value) {
  ...
  render() {
    ...
    header.map(function(item, i) {
      header.length === i + 1
        ? finalHeader.push(
            <span title={item} className={headerClassName} key={i}>
              {item}
            </span>
          )
        : finalHeader.push(
            <span>
              ...
              <span className='headerSeparation'>{separator}</span>
            </span>
          );
    });

    subHeader.map(function(item, i) {
      subHeader.length == i + 1
        ? finalSubHeader.push(
            <span title={item} className={subheaderClassName} key={i}>
              {item}
            </span>
          )
        : finalSubHeader.push(
            <span>
              ...
              <span className='subHeaderSeparation'>{separator}</span>
            </span>
          );
    });
    return (
