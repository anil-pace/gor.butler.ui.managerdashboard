    ...
    function update(data) {
      ...
      svg
        ...
        .on('mousemove', function(d, i) {
          ...
          d3.select('.remove')
            .html(
              "<div style='background:#4d5055; color:#ffffff; padding:10px 30px 10px 10px; border-radius:5%; font-size:14px;'> " +
                d.timeInterval +
                ':00 - ' +
                (d.timeInterval + 1) +
                ":00 <div style='color: #ffffff; padding-top:2px;'> " +
                new Date().getDate() +
                ' ' +
                m_names[new Date().getMonth()] +
                ' ' +
                new Date().getFullYear() +
                "</div> <div style='color: #ffffff; display:inline-block; padding-top:10px; font-size:14px;'> Fulfilled:  " +
                d.type.toLocaleString() +
                " </div> <div style='border-left: 9px solid transparent; border-right: 9px solid transparent; border-top: 12px solid #4d5055; margin-left: 58px; position: absolute; margin-top: 5px;'/>"
            )
      ...
      if (noDataFlag === true) {
        svg
          ...
          .attr('x', width / 2)
          .attr('y', height / 2)
          .text(component.context.intl.formatMessage(messages.noData) || '')
      }
...
var mapDispatchToProps = function(dispatch) {
  return {
    barD3Component: function(data) {
      dispatch(barD3Component(data))
    }
  }
}
