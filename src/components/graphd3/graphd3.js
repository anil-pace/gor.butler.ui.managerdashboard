...
const messages=defineMessages({
    ...
    fulfilled:{
      id:'horizontal.graph.fulfilling',
      description:'Fulfilling',
      defaultMessage:"Fulfilled: {count}"
    }
});
...
"Oct", "Nov", "Dec"); 
      ...
      .on("mousemove", function(d, i) {
        ...
        d3.select('.remove').html( "<div style='background:#4d5055; color:#ffffff; padding:10px 30px 10px 10px; border-radius:5%; font-size:14px;'> "+ d.timeInterval + ":00 - " + (d.timeInterval+1) +":00 <div style='color: #ffffff; padding-top:2px;'> "+ component.context.intl.formatDate(Date.now(),{year:'numeric',month:'long',day:'2-digit'}) +"</div> <div style='color: #ffffff; display:inline-block; padding-top:10px; font-size:14px;'> " +component.context.intl.formatMessage(messages.fulfilled,{count:d.type.toLocaleString()})  + " </div> <div style='border-left: 9px solid transparent; border-right: 9px solid transparent; border-top: 12px solid #4d5055; margin-left: 58px; position: absolute; margin-top: 5px;'/>" ).style("visibility", "visible");
