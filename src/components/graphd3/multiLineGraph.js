    ...
    _graphRender(invData) {
        ...
        if (invData.length > 1) {
            try {
                ...
                var xAxis = d3.svg.axis().scale(x).orient("bottom").tickFormat(d3.time.format("%e")).tickValues(dataArray.map(function (d) {
                    return d.date;
                }))
                ...
                if (noData) {
                    y.domain([0, d3.max(dataArray, function (d) {
                        return config.defaultMaxYAxis
                    })]);
                }
                else {
                    y.domain([0, d3.max(dataArray, function (d) {
                        var maxVal = Math.max(d.items_put, d.items_picked);
