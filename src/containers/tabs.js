...
import {
    OVERVIEW, SYSTEM, ORDERS, USERS, TAB_ROUTE_MAP, INVENTORY, AUDIT,
    FULFILLING_ORDERS, GOR_OFFLINE, GOR_ONLINE, GOR_NORMAL_TAB, GOR_FAIL,
    SOFT_MANUAL, HARD, SOFT, UTILITIES, REPORTS
...
class Tabs extends React.Component {
    ...
    _parseStatus() {
        let overview, system, order, ordersvalue, users, usersvalue, inventoryvalue, overviewClass,
            ...
            offline, systemClass, ordersClass, auditClass, items = {}, auditIcon = false, utilities, reports;
        ...
        if (!this.props.system_status) {
            ...
            if (this.props.overview_status === FULFILLING_ORDERS) {
                overviewStatus =
                    <FormattedMessage id="overviewStatus.tab.fulfilling" description="overview Status fulfilling orders"
            ...
            usersStatus = <FormattedMessage id="usersStatus.tab.heading#" description="users Status "
                                            ...
                                            values={{count: this.props.users_online ? this.props.users_online : "0"}}/>;
        ...
        return (
            <div className="gor-tabs gor-main-block">
                <Link to="/overview" onClick={this.handleTabClick.bind(this, OVERVIEW)}>
                    <Tab items={{tab: items.overview, Status: items.overviewStatus, currentState: items.overviewClass}}
                         changeClass={(this.props.tab.toUpperCase() === OVERVIEW ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={false}/>
                </Link>

                <Link to="/system/butlerbots" onClick={this.handleTabClick.bind(this, SYSTEM)}>
                    <Tab items={{tab: items.system, Status: items.systemStatus, currentState: items.systemClass}}
                         changeClass={(this.props.tab.toUpperCase() === SYSTEM ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={true}/>
                </Link>

                <Link to="/orders/waves" onClick={this.handleTabClick.bind(this, ORDERS)}>
                    <Tab items={{tab: items.order, Status: items.ordersStatus, currentState: items.ordersClass}}
                         changeClass={(this.props.tab.toUpperCase() === ORDERS ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={false}/>
                </Link>


                <Link to="/audit" onClick={this.handleTabClick.bind(this, AUDIT)}>
                    <Tab items={{tab: items.audit, Status: items.auditStatus, currentState: items.auditClass}}
                         changeClass={(this.props.tab.toUpperCase() === AUDIT ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={items.auditIcon}/>
                </Link>

                <Link to="/inventory" onClick={this.handleTabClick.bind(this, INVENTORY)}>
                    <Tab items={{tab: items.inventory, Status: items.inventoryStatus, currentState: ''}}
                         changeClass={(this.props.tab.toUpperCase() === INVENTORY ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={false}/>
                </Link>

                <Link to="/users" onClick={this.handleTabClick.bind(this, USERS)}>
                    <Tab items={{tab: items.users, Status: items.usersStatus, currentState: ''}}
                         changeClass={(this.props.tab.toUpperCase() === USERS ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={false}/>
                </Link>

                {showUtilityTab ? <Link to="/utilities" onClick={this.handleTabClick.bind(this, UTILITIES)}>
                    <Tab items={{tab: items.utilities, Status: '', currentState: ''}}
                         changeClass={(this.props.tab.toUpperCase() === UTILITIES ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={false}/>
                </Link> : ""}
                <Link to="/reports" onClick={this.handleTabClick.bind(this, REPORTS)}>
                    <Tab items={{tab: items.reports, Status: '', currentState: ''}}
                         changeClass={(this.props.tab.toUpperCase() === REPORTS ? 'sel' : GOR_NORMAL_TAB)}
                         subIcons={false}/>
                </Link>
            </div>
        );
