...
import {
  ...
  GOR_ITEMS_PUT,
  GOR_ITEMS_AUDITED,
  PICK_PPS_PERFORMANCE,
  PUT_PPS_PERFORMANCE,
  AUDIT_PPS_PERFORMANCE,
  WS_ONSEND
...
function _getPPSdata(data) {
  ...
  var pps_data = [
    {
      component: { componentNumber: ppsTotal, componentType: Component },
      states: { offState: ppsOff, onState: ppsOn }
    }
  ]
...
function _getButlerdata(data) {
  ...
  var butler_data = [
    {
      component: { componentNumber: butlerTotal, componentType: Component },
      states: { offState: butlerOff, onState: butlerOn }
    }
  ]
...
function _getChargingdata(data) {
  ...
  var charging_data = [
    {
      component: { componentNumber: totalChargers, componentType: Component },
      states: { offState: disconnected, onState: connected }
    }
  ]
  if (data && data.charger_data) {
    if (data.charger_data.connected) connected = data.charger_data.connected
...
class PerformanceWidget extends React.Component {
  ...
  render() {
    ...
    if (this.props.widget === PICK_PPS_PERFORMANCE) {
      if (
        this.props.ppsPerformance.ppsPerformance !== undefined &&
        this.props.ppsPerformance.ppsPerformance.aggregate_data !== undefined
      ) {
        itemRender = (
          <ChartHorizontal
            data={this.props.ppsPerformance}
            type={GOR_ORDER_PICKED}
            performanceParam={this.context.intl.formatMessage(
              messages.performanceParam
            )}
          />
        )
      } else {
    ...
    return (
      <div className="gorPerformanceWidget">
        <div className="gorDrop Performance-Widget-Drop">
          <Dropdown
            optionDispatch={this.props.renderPerformanceWidget}
            items={item}
            styleClass={"ddown"}
        ...
        <div id="performanceGraph">{itemRender}</div>
      </div>
    )
  }
}
...
var mapDispatchToProps = function(dispatch) {
  return {
    ...
    renderPerformanceWidget: function(data) {
      dispatch(renderPerformanceWidget(data))
    }
  }
}
...
export default compose(withQuery)(
  connect(mapStateToProps, mapDispatchToProps)(PerformanceWidget)
)
