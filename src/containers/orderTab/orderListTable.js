...
class OrderListTable extends React.Component {
  ...
  _reqOrderPerPbt(pbtData, saltParams = {}) {
    ...
    try {
      need_to_fetch_more =
        pbtData.ordersPerPbt.order.length < pbtData.ordersPerPbt.totalOrders;
    } catch (ex) {}
  ...
  _formatProgressBar(nr, dr) {
    ...
    if (nr === 0 && dr === 0) {
      ...
      // when ALL orders has been processed
      x.message = (
        <FormattedMessage
  ...
  _processPBTs = nProps => {
    ...
    pbtData = isGroupedById
      ? nProps.props.pbts
      : nProps.props.pbts[0].ordersPerPbt
        ? nProps.props.pbts[0].ordersPerPbt.orders
        : [];
