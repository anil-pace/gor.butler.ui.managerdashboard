...
class OrderListTable extends React.Component {
  ...
  _reqOrderPerPbt(pbtData, saltParams = {}) {
    ...
    try {
      need_to_fetch_more =
        pbtData.ordersPerPbt.orders.length < pbtData.ordersPerPbt.totalOrders;
    } catch (ex) {}

    if (saltParams.lazyData && need_to_fetch_more) {
      // Accordion already open and infinite scroll
      try {
        page = Math.floor(
          parseInt(pbtData.ordersPerPbt.orders.length, 10) / size
        );
        page = page.toString();
      } catch (ex) {
        page = '0';
    ...
    let formData = {
      ...
      filter_applied: this.props.isFilterApplied,
      page: page,
      size: size
    };
    ...
    if (sessionStorage.getItem('filtered_order_status')) {
      formData['filtered_order_status'] = JSON.parse(
        sessionStorage.getItem('filtered_order_status')
      );
    }
    if (sessionStorage.getItem('simple_priority')) {
      formData['simple_priority'] = JSON.parse(
        sessionStorage.getItem('simple_priority')
      );
    }
  ...
  _formatProgressBar(nr, dr) {
    ...
    if (numerator === 0 && denominator === 0) {
      ...
      x.message = (
        <FormattedMessage
          id='orders.toBePicked.status'
          description='status'
          defaultMessage='{total} products picked'
          values={{ total: denominator }}
        />
      );
      x.action = true;
    } else if (numerator === 0) {
      // when ALL are remaining to be picked
      x.message = (
        <FormattedMessage
          id='orders.productsPicked.status'
          description='status'
          defaultMessage='{current} products to be picked'
          values={{ current: denominator }}
        />
      );
      x.action = true;
    } else if (denominator === 0 && numerator > 0) {
      // in case the denominator is less than or equal to 0 because of an issue at the backend.
      x.message = (
        <FormattedMessage
          id='orders.toBePicked.status'
          description='status'
      ...
      x.width =
        Math.ceil(numerator / numerator) > 1
          ? 100
          : (numerator / denominator) * 100;
      x.message = (
        <FormattedMessage
          id='orders.inProgress.status'
          description='status'
          defaultMessage='{current} of {total} products picked'
  ...
  _calculateTimeLeft(cutOffTimeFromBK) {
    ...
    if (cutOffTimeFromBK) {
      ...
      intlLeft = this.props.intl.formatMessage(
        currentLocalTime > cutOffTime ? messages.ago : messages.left
      );
  ...
  _processPBTs = () => {
    ...
    pbtData = isGroupedById
      ? this.props.pbts
      : this.props.pbts[0].ordersPerPbt
      ? this.props.pbts[0].ordersPerPbt.orders
      : [];
    ...
    if (pbtDataLen) {
      for (let i = 0; i < pbtDataLen; i++) {
        ...
        if (!isGroupedById && Object.keys(pbtData[0]).length) {
          formatOrderId =
            pbtData[i].order_id && pbtData[i] !== ''
              ? this.props.intl.formatMessage(messages.orderId, {
                  orderId: pbtData[i].order_id
                })
              : '';

          formatPpsId =
            pbtData[i].pps_id && pbtData[i].pps_id !== ''
              ? this.props.intl.formatMessage(messages.ppsId, {
                  ppsId: pbtData[i].pps_id
                })
              : '';

          formatBinId =
            pbtData[i].pps_bin_id && pbtData[i].pps_bin_id !== ''
              ? this.props.intl.formatMessage(messages.binId, {
                  binId: pbtData[i].pps_bin_id
                })
              : '';

          formatProgressBar = this._formatProgressBar(
            pbtData[i].picked_products_count,
            pbtData[i].total_products_count
          );
          ...
          try {
            if (pbtData[i].start_date) {
              ...
              formatStartDate =
                this._calculateRelativeTime(
                  moment(startDate).tz(timeOffset),
                  moment().tz(timeOffset)
                ) +
                ', ' +
                moment(startDate)
                  .tz(timeOffset)
            ...
            if (pbtData[i].completion_date) {
              ...
              formatCompleteDate =
                this._calculateRelativeTime(
                  moment(completionDate).tz(timeOffset),
                  moment().tz(timeOffset)
                ) +
                ', ' +
          ...
          pbtRow.push(
            <div className='DotSeparatorWrapper'>
              <DotSeparatorContent
                header={[formatOrderId]}
                subHeader={[
                  formatPpsId,
                  formatBinId,
                  formatStartDate,
          ...
          pbtRow.push(
            ...
            >
              ...
              >
                <span>
                  {' '}
                  {this.state.statusMapping[pbtData[i].status]
                    ? this.state.statusMapping[pbtData[i].status]
                    : pbtData[i].status}{' '}
                </span>
                <span>
                  {pbtData[i].missing_count > 0 ? pbtData[i].missing_count : ''}
                </span>
                <span>
                  {pbtData[i].damaged_count > 0 ? pbtData[i].damaged_count : ''}
                </span>
                <span>
                  {pbtData[i].physically_damaged_count > 0
                    ? pbtData[i].physically_damaged_count
                    : ''}
                </span>
                <span>
                  {pbtData[i].unfulfillable_count > 0
                    ? pbtData[i].unfulfillable_count
                    : ''}
                </span>
                <span>
                  {pbtData[i].missing_count > 0 ? pbtData[i].missing_count : ''}
          ...
          if (formatProgressBar.action === true) {
            pbtRow.push(
              <div
                ...
                className='gorButtonWrap'
              >
                <button
                  className='viewOrderLineBtn'
                  onClick={() => this._viewOrderLine(pbtData[i].order_id)}
                >
                  <FormattedMessage
                    id='orders.view.orderLines'
                    description='button label for view orderlines'
                    defaultMessage='VIEW ORDERLINES '
                  />
                </button>
                {/* if order_priority is NOT null, show embeddedImage */}
                {pbtData[i].order_priority ? (
                  <div
                    className='embeddedImage'
                    onClick={() =>
                      this._getOrderPriorityList(pbtData[i].order_id)
                    }
                  />
                ) : (
                  ''
                ...
                this.state.orderIdForOrderPriority === pbtData[i].order_id ? (
                  <OrderPriority
          ...
          let formatIntlPbt = pbtData[i].cut_off_time
            ? moment(pbtData[i].cut_off_time)
                .tz(timeOffset)
                .format('HH:mm')
            : '';
          let formatPbtTime = pbtData[i].cut_off_time
            ? this.props.intl.formatMessage(messages.cutOffTime, {
                cutOffTime: formatIntlPbt
              })
            : 'NO CUT OFF TIME';
          let formatTimeLeft = this._calculateTimeLeft(pbtData[i].cut_off_time);
          let formatProgressBar = this._formatProgressBar(
            pbtData[i].picked_products_count,
            pbtData[i].total_products_count
          );
          let formatTotalOrders = (
            <FormattedMessage
              id='orders.total'
              description='total orders'
              defaultMessage='Total {total} orders'
          ...
          pbtRow.push(
            <div className='DotSeparatorWrapper'>
              {formatTimeLeft ? (
                <DotSeparatorContent
                  header={[formatPbtTime]}
                  subHeader={[formatTimeLeft]}
                  key={i}
                />
              ) : (
                <DotSeparatorContent
          ...
          pbtRow.push(
            <div className='totalOrderWrapper'>{formatTotalOrders}</div>
          );
          pbtRows.push(pbtRow);
        } else {
          return [];
  ...
  _processOrders = orderData => {
    ...
    if (orderDataLen) {
      for (let i = 0; i < orderDataLen; i++) {
        ...
        formatOrderId =
          orderData[i].order_id && orderData[i] !== ''
            ? this.props.intl.formatMessage(messages.orderId, {
        ...
        formatPpsId =
          orderData[i].pps_id && orderData[i].pps_id !== ''
            ? this.props.intl.formatMessage(messages.ppsId, {
                ppsId: orderData[i].pps_id
              })
            : '';

        formatBinId =
          orderData[i].pps_bin_id && orderData[i].pps_bin_id !== ''
            ? this.props.intl.formatMessage(messages.binId, {
                binId: orderData[i].pps_bin_id
              })
            : '';

        formatProgressBar = this._formatProgressBar(
          orderData[i].picked_products_count,
          orderData[i].total_products_count
        );
        ...
        orderRow.push(
          ...
          >
            ...
            >
              <span>
                {' '}
                {this.state.statusMapping[orderData[i].status]
                  ? this.state.statusMapping[orderData[i].status]
                  : orderData[i].status}{' '}
              </span>
              <span>
                {' '}
                {orderData[i].missing_count > 0
                  ? orderData[i].missing_count
                  : ''}{' '}
              </span>
              <span>
                {' '}
                {orderData[i].damaged_count > 0
                  ? orderData[i].damaged_count
                  : ''}{' '}
              </span>
              <span>
                {' '}
                {orderData[i].physically_damaged_count > 0
                  ? orderData[i].physically_damaged_count
                  : ''}{' '}
              </span>
              <span>
                {' '}
                {orderData[i].unfulfillable_count > 0
                  ? orderData[i].unfulfillable_count
                  : ''}{' '}
              </span>
              <span>
                {' '}
        ...
        if (formatProgressBar.action === true) {
          orderRow.push(
            ...
            >
              <button
                onClick={() => this._viewOrderLine(orderData[i].order_id)}
              >
                <FormattedMessage
                  id='orders.view.orderLines'
                  description='button label for view orderlines'
              ...
              {/*if order_priority is NOT null, show embeddedImage */}
              {orderData[i].order_priority ? (
                <div
                  className='embeddedImage'
                  onClick={() =>
                    this._getOrderPriorityList(orderData[i].order_id)
                  }
                />
              ) : (
                ''
              ...
              {this.state.isOrderPriorityIconClicked &&
              this.state.orderIdForOrderPriority === orderData[i].order_id ? (
                <OrderPriority
                  onClick={this._callBack}
                  showOrderPriorityList={this.state.showOrderPriorityList}
  ...
  _onScrollHandler(pbtData, event) {
    if (
      pbtData.ordersPerPbt &&
      pbtData.ordersPerPbt.totalOrders > pbtData.ordersPerPbt.orders.length
    ) {
      if (
        Math.round(event.target.scrollTop) +
          Number(event.target.clientHeight) ===
        Number(event.target.scrollHeight)
      ) {
          ...
          <GTable options={['table-bordered']}>
            {processedPbtData ? (
              <GTableBody data={processedPbtData.pbtData}>
                {processedPbtData.pbtData
                  ? processedPbtData.pbtData.map(function(row, idx) {
                      return isGroupedById ? (
                        <Accordion
                          ...
                          onScrollHandler={self._onScrollHandler.bind(
                            self,
                            self.props.pbts[idx]
                          )}
                          ...
                          title={
                            <GTableRow
                              ...
                              key={idx}
                              index={idx}
                              offset={processedPbtData.offset}
                              max={processedPbtData.max}
                              data={processedPbtData.pbtData}
                            >
                        ...
                        >
                          ...
                          {self.props.pbts[idx].ordersPerPbt && (
                            <GTableBody
                              data={
                                self._processOrders(
                                  self.props.pbts[idx].ordersPerPbt.orders
                                ).orderData
                              }
                            >
                              {self
                                ...
                                .orderData.map(function(row_1, idx_1) {
                                  return (
                                    <GTableRow
                                      key={idx_1}
                                      index={idx_1}
                                    ...
                                    >
                                      {Object.keys(row_1).map(function(
                                        text,
                                        index
                                      ) {
                                        return (
