...
class OrderListTab extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (JSON.stringify(nextProps.location.query) !== JSON.stringify(this.state.query)) {
			this.setState(
				{
	...
	_refreshList(query) {
		...
		if (!query.fromDate) {
			query.fromDate = moment.tz(timeOffset).startOf('day').format('YYYY-MM-DD');
		}
		if (!query.toDate) {
			query.toDate = moment.tz(timeOffset).endOf('day').format('YYYY-MM-DD');
		}
		if (!query.fromTime) {
			query.fromTime = moment.tz(timeOffset).startOf('day').format('HH:mm:ss');
		}
		if (!query.toTime) {
	...
	_handleClickRefreshButton() {
		this._refreshList(
			this.state.query,
			this.props.orderSortHeaderState ? this.props.orderSortHeaderState.colSortDirs : null
		);
	}
	/**
   ...
   */
	...
	render() {
		...
		const durDiff = moment
			.duration(
				moment(this.state.endDateForOrders)
					.tz(timeOffset)
					.diff(moment(this.state.startDateForOrders).tz(timeOffset))
			)
			.asDays();
...
export default connect(mapStateToProps, mapDispatchToProps)(injectIntl(OrderListTab));
