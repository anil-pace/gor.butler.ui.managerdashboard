...
class OrderListTab extends React.Component {
  ...
  _reqCutOffTime(
    ...
    if (filteredOrderPriority) {
      ...
      sessionStorage.setItem(
        'simple_priority',
        JSON.stringify(filteredOrderPriority)
      );
    }
  ...
  _refreshList(query) {
    ...
    this.setState(
      ...
      () => {
        ...
        this.props.filterApplied(query.filter_applied);
      }
    );
  }
  ...
  _setPolling() {
    ...
    timerId = setInterval(function () {
      ...
      query.filteredOrderPriority = JSON.parse(
  ...
  render() {
    ...
    if (Math.floor(durDiff) > 0) {
      ...
      duration = moment(this.state.startDateForOrders)
        .tz(timeOffset)
        .format('DD MMM YYYY');
    }
    ...
    return (
      <div>
        <div className='gor-Orderlist-table'>
          ...
          {!this.props.orderListSpinner && this.props.pbts.length === 0 && (
