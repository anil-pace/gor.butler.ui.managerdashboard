...
import {BASE_URL, ORDERS_URL,PAGE_SIZE_URL,PROTOCOL} from '../../constants/configConstants';
...
class OrderListTab extends React.Component{
  ...
  componentDidMount() {
    ...
    if(data.url === undefined) {
      url = PROTOCOL+BASE_URL + ORDERS_URL + "?page=" + (data.selected+1) + "&PAGE_SIZE=25";
    }
   ...
   refresh() {
    ...
    data.selected = 0;
    data.url = "";
    data.url = PROTOCOL+BASE_URL + ORDERS_URL + "?page=" + (data.selected+1);
