...
class OrderFilter extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.orderFilterState &&
      JSON.stringify(this.state) !== JSON.stringify(nextProps.orderFilterState)
    ) {
  ...
  _processFilterToken() {
    var tokenField1 = {
      ...
      label: (
        <FormattedMessage id='order.token.status_1' defaultMessage='PRIORITY' />
      )
    };
    var tokenField2 = {
      value: 'STATUS',
      label: (
        <FormattedMessage id='order.token.status' defaultMessage='STATUS' />
      )
    };
    ...
    var labelC1 = [
      {
        value: ANY,
        label: <FormattedMessage id='order.priority.any' defaultMessage='Any' />
      },
      {
        value: HIGH_PRIORITY,
        label: (
    ...
    var labelC2 = [
      {
        value: ANY,
        label: <FormattedMessage id='order.status.any' defaultMessage='Any' />
      },
      {
    ...
    var columnDetail = { column2token: column2, column1token: column1 };
  ...
  _applyFilter(doNotShowFilterStatus) {
    ...
    if (filterState.searchQuery['FROM DATE']) {
      _query.fromDate = filterState.searchQuery['FROM DATE'];
    }

    if (filterState.searchQuery['FROM TIME']) {
      _query.fromTime = filterState.searchQuery['FROM TIME'];
    }

    if (filterState.searchQuery['TO DATE']) {
      _query.toDate = filterState.searchQuery['TO DATE'];
    }

    if (filterState.searchQuery['TO TIME']) {
      _query.toTime = filterState.searchQuery['TO TIME'];
    }

    if (
      filterState.tokenSelected[ORDER_PRIORITY] &&
      filterState.tokenSelected[ORDER_PRIORITY][0] !== ANY
    ) {
      ...
      <div>
        <Filter>
          ...
          <div className='gor-filter-body'>
            ...
            <div className='gor-filter-body-input-wrap'>{ppsIdSearchField}</div>

            <div
              style={{ height: '10em', paddingTop: '1em' }}
              className='gor-filter-body-input-wrap'
            >
