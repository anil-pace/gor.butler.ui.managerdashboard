...
class ViewOrderLine extends React.Component {
  ...
  _getOrdersLines = orderId => {
    let params = {
      url: ORDERLINES_PER_ORDER_URL + '?id=' + orderId,
      method: GET,
  ...
  handleChange(event) {
    ...
    updatedList.forEach(function(item) {
      let pdfa_values_split = item.pdfa_values[0]
        ? item.pdfa_values[0]
            .substring(1, item.pdfa_values[0].length - 1)
            .split(',')
        : [];
  ...
  _processOLHeader = arg => {
    ...
    let olineHeader = [];
    let formatOLTotal = arg.total_orderlines
      ? this.props.intl.formatMessage(messages.totalOrderLines, {
          totalOrderlines: arg.total_orderlines
        })
      : 'null';

    olineHeader.push(
      <div style={{ marginLeft: '20px' }} className='DotSeparatorWrapper'>
        <DotSeparatorContent
          header={['Order Lines']}
          subHeader={[formatOLTotal]}
        />
      </div>
    );

    let formatProgressBar = this._formatProgressBar(
      arg.picked_products_count,
      arg.total_products_count
    );
    olineHeader.push(
      <div>
        ...
        <div style={{ paddingTop: '10px', color: '#333333', fontSize: '14px' }}>
          {' '}
  ...
  _processOLList = arg => {
    ...
    if (olDataLen) {
      for (let i = 0; i < olDataLen; i++) {
        let olineRow = [];
        let subHeaderData = [];
        let pdfa_values_split = arg[i].pdfa_values[0]
          .substring(1, arg[i].pdfa_values[0].length - 1)
          .split(','); // converts to string and then splits
        let pdfa_values_split_sku = pdfa_values_split[0].split('='); // splits first element to extract sku id
        let skuId = pdfa_values_split_sku[1].substring(
          2,
          pdfa_values_split_sku[1].length - 1
        );
        let formatSkuId = this.props.intl.formatMessage(messages.skuId, {
          skuId: skuId
        });
        ...
        olineRow.push(
          <div style={{ marginLeft: '20px' }} className='DotSeparatorWrapper'>
            <DotSeparatorContent
              header={[formatSkuId]}
              headerClassName='viewOrderLinesHeader'
              subHeader={subHeaderData}
              subheaderClassName='subheaderName viewOrderLinesSubHeader'
              separator={<span className='straightLine'>|</span>}
            />
          </div>
        );

        let formatProgressBar = this._formatProgressBar(
          arg[i].pick_products_count,
          arg[i].total_products_count
        );
        olineRow.push(
          <div
            style={{
        ...
        olineRow.push(
          ...
          >
            <span>
              {' '}
              {this.state.statusMapping[arg[i].status]
                ? this.state.statusMapping[arg[i].status]
                : arg[i].status}{' '}
            </span>
            <span>
              {' '}
              {arg[i].missing_count > 0 ? arg[i].missing_count : ''}{' '}
            </span>
            <span>
              {' '}
              {arg[i].damaged_count > 0 ? arg[i].damaged_count : ''}{' '}
            </span>
            <span>
              {' '}
              {arg[i].physically_damaged_count > 0
                ? arg[i].physically_damaged_count
                : ''}{' '}
            </span>
            <span>
              {' '}
              {arg[i].unfulfillable_count > 0
                ? arg[i].unfulfillable_count
                : ''}{' '}
            </span>
            <span>
              {' '}
  ...
  _calculateTimeLeft(cutOffTimeFromBK) {
    ...
    if (cutOffTimeFromBK) {
      ...
      if (diff > 3600000) {
        // 3600 * 1000 milliseconds is for 1 hr
        timeLeft = Math.floor(diff / 3600000) + ' hrs left';
      } else if (diff > 60000) {
        // 60 *1000 milliseconds is for 1 min
        timeLeft = Math.floor(diff / 60000) + ' mins left';
      } else if (diff > 1000) {
        // 1000 milliseconds is for 1 sec
  ...
  render() {
    ...
    let formatCutOffTime = this.props.orderLines.cut_off_time ? (
      <FormattedMessage
        id='orders.orderlines.cutofftime'
        description='orderlines cut off time'
    ...
    let formatTimeLeft = this._calculateTimeLeft(
      this.props.orderLines.cut_off_time
    );
          ...
          <div className='gor-modal-body'>
            <div className='orderDetailsWrapper'>
              <div className='orderDetailsHeader'>
                <FormattedMessage
                  id='orders.oLines.oDetails'
                  description='Heading for Order details'
                ...
                <div className='orderDetailsLeft'>
                  <div className='orderDetailsColumn'>
                    <div className='orderDetailsRows'>
                      <span className='spanKeys col-1-span-key'>
                        {' '}
                        <FormattedMessage
                          id='orders.oLines.ppsId'
                          description='pps id'
                          defaultMessage='PPS ID:'
                        />{' '}
                      </span>
                      <span className='spanValues'>
                        {' '}
                        {this.props.orderLines.pps_id}{' '}
                      </span>
                    </div>
                    <div className='orderDetailsRows'>
                      <span className='spanKeys col-1-span-key'>
                        {' '}
                        <FormattedMessage
                          id='orders.oLines.binNo'
                          description='bin no'
                          defaultMessage='Bin no:'
                        />{' '}
                      </span>
                      <span className='spanValues'>
                        {' '}
                        {this.props.orderLines.pps_bin_id}{' '}
                      </span>
                    </div>
                    {this.props.orderLines.username ? (
                      ...
                      <div className='orderDetailsRows'> &nbsp;</div>
                    )}
                  </div>

                  <div className='orderDetailsColumn'>
                    <div className='orderDetailsRows'>
                      <span className='spanKeys col-2-span-key'>
                        {' '}
                        <FormattedMessage
                          id='orders.oLines.pending'
                          description='lines received'
                          defaultMessage='Lines received:'
                        />{' '}
                      </span>
                      <span className='spanValues'>
                        {' '}
                        {this.props.orderLines.total_orderlines}{' '}
                      </span>
                    </div>
                    <div className='orderDetailsRows'>
                      <span className='spanKeys col-2-span-key'>
                        {' '}
                        <FormattedMessage
                          id='orders.oLines.linesinprogress'
                          description='lines in progress'
                          defaultMessage='Lines in progress:'
                        />{' '}
                      </span>
                      <span className='spanValues'>
                        {' '}
                        {this.props.orderLines.pending_orderlines}{' '}
                      </span>
                    </div>
                    <div className='orderDetailsRows'>
                      <span className='spanKeys col-2-span-key'>
                        {' '}
                        <FormattedMessage
                          id='orders.oLines.linescompleted'
                          description='lines completed'
                          defaultMessage='Lines completed:'
                        />{' '}
                      </span>
                      <span className='spanValues'>
                        {' '}
                        {this.props.orderLines.completed_orderlines}{' '}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className='orderDetailsColumn'>
                    <div className='orderDetailsRows'>
                      <span className='spanKeys col-3-span-key'>
                        {' '}
                        <FormattedMessage
                          id='orders.oLines.cutofftime'
            ...
            <div className='orderDetailsListWrapper'>
              <GTable options={['table-bordered']}>
                <GTableBody data={processedHeader.olineHeader}>
                  {processedHeader.olineHeader
                    ? processedHeader.olineHeader.map(function(row, idx) {
                        return (
                          <GTableRow
                            key={idx}
                            index={idx}
                            offset={processedHeader.offset}
                            max={processedHeader.max}
                            data={row}
                          >
                ...
                <GTableBody data={processedList.olineData}>
                  {' '}
                  {/*<GTableBody data={this.state.items} >*/}
                  {processedList.olineData
                    ? processedList.olineData.map(function(row, idx) {
                        return (
                          <GTableRow
                            key={idx}
                            index={idx}
                            offset={processedList.offset}
                            max={processedList.max}
                            data={row}
                          >
                            {Object.keys(row).map(function(text, index) {
                              return (
