...
class UsersTab extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let legacyDataSubscribed = false
    ...
    this.setState(
      () => {
        return {
          legacyDataSubscribed,
          UserList: nextProps.data.UserList.list
        }
      },
      () => {
        if (legacyDataSubscribed) {
  ...
  _processUserDetails() {
    ...
    for (var i = data.length - 1; i >= 0; i--) {
      ...
      if (data[i].logged_in) {
        ...
        if (data[i].pps_mode && data[i].seat_type) {
          userData.workMode =
            work_mode[data[i].pps_mode] + " " + work_place[data[i].seat_type]
        } else if (data[i].pps_mode) {
          userData.workMode = work_mode[data[i].pps_mode]
        }
