...
class UserFilter extends React.Component {
  ...
  _processUserRoll() {
    let objRole = [
        {
          value: 'all',
          label: <FormattedMessage id='user.role.all' defaultMessage='Any' />
        }
      ],
      roleData = {},
      currentRole;
  ...
  _processFilterToken() {
    let tokenStatus = {
      value: 'STATUS',
      label: (
        <FormattedMessage id='user.tokenfield.status' defaultMessage='STATUS' />
      )
    };
    let tokenRole = {
      value: 'ROLE',
      label: (
        <FormattedMessage id='user.tokenfield.role' defaultMessage='ROLE' />
      )
    };
    let tokenWorkMode = {
      value: 'WORK_MODE',
    ...
    const labelC1 = [
      {
        value: 'all',
        label: <FormattedMessage id='user.status.all' defaultMessage='Any' />
      },
      {
        value: 'online',
        label: (
          <FormattedMessage id='user.status.online' defaultMessage='Online' />
        )
      },
      {
        value: 'offline',
        label: (
          <FormattedMessage id='user.status.offline' defaultMessage='Offline' />
        )
      }
    ];
    ...
    const labelC3 = [
      {
        value: 'all',
        label: <FormattedMessage id='user.workmode.all' defaultMessage='Any' />
      },
      {
        value: 'pick__front',
        label: (
      ...
      },
      {
        value: 'audit',
        label: (
          <FormattedMessage id='user.workmode.audit' defaultMessage='Audit' />
        )
      },
      {
        value: 'management',
  ...
  _applyFilter() {
    ...
    if (filterState.tokenSelected) {
      ...
      filterState.tokenSelected['STATUS'].length !== 2
        ? (filterSubsData['logged_in'] = [
            'is',
            filterState.tokenSelected['STATUS'][0] === 'online'
              ? 'true'
              : 'false'
          ])
        : '';
      filterState.tokenSelected['ROLE'] &&
      filterState.tokenSelected['ROLE'][0] !== 'all'
        ? (filterSubsData['role'] = ['in', filterState.tokenSelected['ROLE']])
        : '';
      /** Gaurav Makkar:
       ...
       */
      if (
        filterState.tokenSelected['WORK_MODE'] &&
        filterState.tokenSelected['WORK_MODE'][0] !== 'all'
      ) {
        let pps_list = [];
      ...
      <div>
        <Filter>
          ...
          <div className='gor-filter-body'>
            <div className='gor-filter-body-input-wrap'>{userSearchField}</div>
...
export default compose(withRoleList)(connect(mapStateToProps)(UserFilter));
