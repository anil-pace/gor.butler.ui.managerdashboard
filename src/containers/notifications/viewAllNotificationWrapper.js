...
import {
  ...
  APP_JSON,
  DEFAULT_NOTIFICATION_ROW_LENGTH,
  DESC,
  ASC
...
class ViewAllNotificationWrapper extends React.Component {
  ...
  _processData() {
    var data = this.props.searchAppliedAllNotifications
      ? this.props.searchedAllNotificationData
      : this.props.completeNotificationData
    ...
    if (dataLen) {
      for (let i = 0; i < dataLen; i++) {
        ...
        let createDate = new Date(
          new Date(data[i].createTime).toLocaleString("en-US", {
        ...
        tableRow.push(
          <div>
            <span className={data[i].type + "-icon"}></span>
            <span style={{ "margin-left": "10px" }}>
              <DotSeparatorContent
                header={[
                  data[i].component ? data[i].component : "",
                  data[i].title
                ]}
        ...
        tableRow.push(
          <div className="descNotification">{data[i].description}</div>
        )
    ...
    processedData.header = [
      ...
      },
      {
        ...
        sortable: true,
        defaultOrder: this.state.order ? DESC : ASC
      }
    ]
  ...
  _onScrollHandler(event) {
    if (
      ...
      let page =
        this.state.dataFound === false ? this.state.page : this.state.page + 1
      this.setState(
        {
          page
        },
      ...
      <div className="allNotificationWrap">
        ...
        <div className="gor-modal-content">
          <div className="gor-modal-head">
            <span className="close" onClick={this._closeModal.bind(this)}>
              ×
            </span>
            <p className="modal-head-text">
              <FormattedMessage
            ...
            >
              <button
                ...
                onClick={self._setFilter.bind(self)}
                disabled
              >
                <div className="gor-manage-task" />
                <FormattedMessage
                  id="gor.filter.filterLabelcaps"
            ...
            <div className="gorTableMainContainer gor-all-not-tbl">
              ...
              <GTable options={["table-bordered", "topLineBorder"]}>
                <GTableBody
                  ...
                  onScrollHandler={self._onScrollHandler.bind(this)}
                >
...
function mapStateToProps(state, ownProps) {
  return {
    completeNotificationData:
      state.notificationReducer.completeNotificationData || [],
    hasDataChanged: state.notificationReducer.hasDataChanged,
    isLoading: state.notificationReducer.isLoading,
    searchedAllNotificationData:
      state.notificationReducer.searchedAllNotificationData,
    searchAppliedAllNotifications:
      state.notificationReducer.searchAppliedAllNotifications,
...
function mapDispatchToProps(dispatch) {
  return {
    getNotificationData: function(params) {
      dispatch(getNotificationData(params))
    },
    resetNotificationTableData: function(data) {
      dispatch(resetNotificationTableData(data))
    },
    setNotificationSpinner: function(data) {
      dispatch(setNotificationSpinner(data))
    },
