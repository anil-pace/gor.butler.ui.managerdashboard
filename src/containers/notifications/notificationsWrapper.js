...
class NotificationsWrapper extends React.Component {
  ...
  _onScrollHandler(event) {
    if (
      this.props.searchApplied &&
      event.target.scrollHeight - event.target.scrollTop ===
        event.target.clientHeight
    ) {
      let page =
        this.props.searchDataFound === false
          ? this.state.page
          : this.state.page + 1
      this.setState(
        {
  ...
  render() {
    var notificationData = this.props.searchApplied
      ? this.props.searchedNotificationData
      : this.props.wsNotificationData
    ...
    return (
      <div className="notificationWrap">
        <Notification
          onScrollHandler={this._onScrollHandler.bind(this)}
...
function mapStateToProps(state, ownProps) {
  return {
    searchedNotificationData:
      state.notificationReducer.searchedNotificationData || [],
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    wsNotificationInit: function() {
      dispatch(wsNotificationInit())
    },
    resetNotificationData: function() {
      dispatch(resetNotificationData())
    },
    wsNotificationSubscribe: function(data) {
      dispatch(wsNotificationSubscribe(data))
    }
  }
}
