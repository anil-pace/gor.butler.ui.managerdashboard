...
import {
  AUDIT_DETAILS_QUERY,
  AUDIT_DETAILS_SUBSCRIPTION_QUERY
...
const messages = defineMessages({
  ...
  vdExtraFound: {
    ...
    defaultMessage: "extra entity found"
  },
  vdDamagedFound: {
    id: "viewDetais.damaged.status",
    defaultMessage: "damaged entity found"
  },
  vdItems: {
...
class viewDetailsAudit extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.client
      ...
      .then((data, error) => {
        ...
        this.setState({
          attributeData: data.data.AuditDetails.list.entity_list || []
        })
  ...
  _timeFormat(UTCtime) {
    ...
    if (UTCtime) {
      if (getDaysDiff(UTCtime) < 2) {
        time =
          this.context.intl.formatRelative(UTCtime, {
            timeZone: timeOffset,
            units: "day"
          }) +
          ", " +
  ...
  _processDataTile(data) {
    ...
    let vdMultiLocation = this.context.intl.formatMessage(
      messages.vdMultiLocation
    )
    let vdSingleLocation = this.context.intl.formatMessage(
      messages.vdSingleLocation
    )
    let vdLinesCompleted = this.context.intl.formatMessage(
      messages.vdLinesCompleted
    )
    ...
    if (data.audit_param_type == "sku") {
      tile1Data[vdhAuditType] =
        data.entity_list.length > 1 ? vdMultiSKU : vdSingleSKU
    } else if (data.audit_param_type == "location") {
      tile1Data[vdhAuditType] =
        data.entity_list.length > 1 ? vdMultiLocation : vdSingleLocation
    }
    if (data.pps_id && data.pps_id.length > 1) {
      ppsList = (
        <span>
          ...
          <span
            title={data.pps_id}
            className="gor-view-details-info-icon"
          ></span>
        </span>
      )
    } else {
    ...
    tile3Data[vdhShowKQ] = data.kq.toString()
  ...
  processData(itemsData) {
    let vdAttrSelected = this.context.intl.formatMessage(
      messages.vdAttrSelected
    )
    let vdNoAttrSelected = this.context.intl.formatMessage(
      messages.vdNoAttrSelected
    )
    ...
    let vdDamagedFound = this.context.intl.formatMessage(
      messages.vdDamagedFound
    )
    ...
    for (var i = 0; i < itemsData.length; i++) {
      ...
      if (itemsData[i].result.sku_status == "inventory_empty") {
        ...
        if (items.damaged_quantity > 0) {
          rowObject.status = items.damaged_quantity + " " + vdDamagedFound
        } else {
          if (diff > 0) {
            rowObject.status =
              diff + " " + vdMissingOut + " " + items.expected_quantity
          } else {
            rowObject.status =
              diff !== 0 ? Math.abs(diff) + " " + vdExtraFound : ""
          }
        }
      }
        ...
        <div>
          <div>
            <div className="gor-Auditlist-table">
              {
                <Spinner
                  isLoading={this.props.viewAuditSpinner}
          ...
          <div className="gor-auditDetails-modal-head">
            <span className="AuditIDWrapper">
              <FormattedMessage
                ...
                defaultMessage="Audit Task"
              />
            </span>
            <span className="AuditIDWrapper">- {this.props.displayId}</span>
          ...
          <div className="gor-auditDetails-modal-body">
            <div className="AuditDetailsWrapper">
              <div className="AuditDetailsHeader">
                <FormattedMessage
                  id="audit.auditdetails"
                  description="Heading for audit details"
              ...
              <div className="auditDetailsContent">
                <div className="auditDetailsLeft">
                  ...
                  {this.state.auditDetails.change_pps_button == "enable" ? (
                    <div
                      className="details-changepps"
                      onClick={this.ppsChange.bind(this)}
                    >
                      | {vdChangePPS}
                    </div>
                  ) : (
          ...
          <GTable options={["table-bordered", "viewDetailsTable"]}>
            <GTableHeader options={["auditTable"]}>
              <GTableHeaderCell key={1} header={audit}>
                <span className="auditSummary">
                  {no_of_record} {type} {auditTask}
                </span>
                <div className="auditDetailsSearchWrap">
                  <SearchFilter
                    handleChange={this.handleChange}
                    placeHolder={vdSearchBySKU}
                  />
                </div>
              </GTableHeaderCell>
            </GTableHeader>
            {processedTableData.length > 0 ? (
              <GTableBody data={processedTableData}>
                {processedTableData
                  ? processedTableData.map(function(row, idx) {
                      return (
                        <GTableRow
                          key={idx}
                          index={idx}
                            ...
                            return (
                              ...
                              >
                                {index == 0 ? (
                                  <DotSeparatorContent
                                    header={
                                      processedTableData[idx][text]["header"]
                                    }
                                    subHeader={
                                      processedTableData[idx][text]["subHeader"]
                                    }
                                    separator={<div className="dotImage"></div>}
                                    subheaderClassName="subheaderName viewDetailslimitsSubHeader"
                                  />
                                ) : (
                                  ""
                                )}
                                {index == 1 ? (
                                  <DotSeparatorContent
                                    ...
                                    headerClassName="viewDetailsSeparatorHeader"
                                    subheaderClassName="viewDetailsSeparatorSubHeader"
                                    separator={<div className="dotImage"></div>}
                                  />
                                ) : (
                                  ""
              ...
              <div
                className="gor-Audit-no-dataview"
