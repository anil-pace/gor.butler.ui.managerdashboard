...
import { auditNeedRefreshFlag, auditEditDupData } from './query/clientQuery';
...
const messages = defineMessages({
  ...
  wallToWallSubheaderAudit: {
    id: 'audit.walltowallSubHeaderAudit.text',
    description:
      'Audit all locations; no order fulfilment can happen during this audit operation',
    defaultMessage:
      'Audit all locations; no order fulfilment can happen during this audit operation'
  },
...
class CreateAudit extends React.Component {
  constructor(props) {
    ...
    this._otherLocation = this._otherLocation.bind(this);
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.hasDataChanged !== nextProps.hasDataChanged) {
      let locationAttributes =
          this.props.locationDataChanged !== nextProps.locationDataChanged
            ? JSON.parse(JSON.stringify(nextProps.locationAttributes))
            : {},
        skuAttributes =
          this.props.skuDataChanged !== nextProps.skuDataChanged
            ? JSON.parse(JSON.stringify(nextProps.skuAttributes))
            : {},
        validatedLocations =
  ...
  _validateSKU(type) {
    ...
    if (type === 'validate') {
      ...
      if (!isAttributeSelected && type === 'create') {
        ...
        validSKUData.action =
          type === 'create' || type === 'confirm' ? 'create' : '';
        ...
        for (let i = 0, len = skuList.length; i < len; i++) {
          if (selectedSKUList[skuList[i].value]) {
            validSKUData.audit_param_value.push(
            ...
            validSKUData.audit_param_value.push(noAttributeSKU);
          }
        }
        sendRequest = true;
        this.props.removeModal();
    ...
    if (sendRequest) {
      ...
      dataToSent = {
        sku: {
          ...
          audit_creator_name: validSKUData.audit_creator_name,
          data: JSON.stringify(validSKUData.audit_param_value),
          kq: validSKUData.kq
        }
      };
      ...
      this.props.client
        ...
        .then(data => {
          ...
          if (auditEditData.action == 'create') {
            AuditParse(auditEditData, auditEditData.action, _this);
          } else {
      ...
      this.props.client
        ...
        .then(data => {
          ...
          var values = {},
            ...
            msg = {};
          if (auditEditData.action == 'create') {
            if (auditEditData.display_id) {
              values = { id: auditEditData.display_id };
              msg = getFormattedMessages('CREATEAUDIT', values);
              _this.props.notifyfeedback(msg);
              return;
            } else {
              stringInfo = codeToString(auditEditData.alert_data[0]);
  ...
  _otherLocation() {
    let validLocationDataCreateAudit;
    validLocationDataCreateAudit = {
      sku: {
        audit_param_name: this.auditNameOther.value,
        audit_param_type: 'location',
        audit_creator_name: this.props.username || 'admin',
        action: 'create',
        kq: this.kqCheckOtherLocation.checked,
        data: JSON.stringify([]),
        audit_frequency: '',
        reminder: ''
      }
    ...
    this.props.client
      .query({
        query: AUDIT_VALIDATE_QUERY,
        variables: validLocationDataCreateAudit,
        fetchPolicy: 'network-only'
      })
      .then(data => {});
  ...
  _togglePanel() {
    this.setState({
      panelClass:
        this.state.panelClass === 'expanded' ? 'collapsed' : 'expanded'
    });
  }
  ...
  render() {
    ...
    let otherAuditMessg = (
      <FormattedMessage
        id='audit.auditother.text'
        description='text for other audits'
        defaultMessage='Other Audits'
      />
    );
    let selectAllLabel = (
      <FormattedMessage
    ...
    let wallToWallSubheaderAudit = this.props.intl.formatMessage(
      messages.wallToWallSubheaderAudit
    );
          ...
          <div className='gor-modal-body'>
            <div className='gor-audit-form new-audit'>
              ...
              >
                        ...
                        <Tab
                          tabName={
                            <span className={'sub-tab-name'}>
                              ...
                              {skuCSVSelected ? enterCSVMessg : enterSkuMessg}
                            </span>
                          }
                          iconClassName={'icon-class-0'}
                          linkClassName={'link-class-0'}
                        >
                          {!validationDoneSKU && !skuCSVSelected && (
                            <div>
                              <div>
                                      ...
                                      return tuple.visible ? (
                                        ...
                                        >
                                          <InputComponent.CopyPaste
                                            ...
                                            value={tuple.value}
                                            placeholder={self.props.intl.formatMessage(
                                              messages.auditskuinputplaceholder
                                            )}
                                          />
                                        </div>
                                      ) : null;
                          ...
                          {validationDoneSKU && (
                            <div>
                              <div className={'gor-audit-inputlist-wrap'}>
                                <div
                                  className={
                                    'gor-audit-inputlist-cont ' +
                                    self.state.panelClass
                                  }
                                >
                                  ...
                                  <div>
                                    {copyPasteSKU.data.map((tuple, i) => {
                                      let tuples = [],
                                        attributeList = this.state.skuAttributes
                                          .data[i].attributeList;
                                      if (tuple.visible) {
                                        tuples.push(
                                          <div
                                            className={
                              ...
                              <div className='gor-audit-excol-wrap'>
                                {' '}
                                <span
                                  onClick={this._togglePanel}
                                  className={
                                    'gor-exp-coll ' + self.state.panelClass
                                  }
                                />
                              </div>
                            </div>
                ...
                >
                  <div>
                    ...
                    >
                      ...
                      {!validationDone ? (
                        <div className='gor-audit-location-wrap'>
                          <div className='gor-sub-head-audit-input'>
                            <FormattedMessage
                              ...
                              return tuple.visible ? (
                                ...
                                >
                                  <InputComponent.CopyPaste
                                    ...
                                    value={tuple.value}
                                    placeholder={self.props.intl.formatMessage(
                                      messages.auditlocationinputplaceholder
                                    )}
                                  />
                                </div>
                              ) : null;
                      ...
                      <div className={'gor-sku-validation-btn-wrap'}>
                        <button
                          ...
                          type='button'
                          onClick={this._validateLocation}
                        >
                          <label>
                            {!validationDone && validateclicked ? (
                              <div className='gor-spinner' />
                            ) : (
                              <FormattedMessage
                                id='audits.validateSKU'
                                description='Text for validate sku button'
                    ...
                    <section className={'set-kq-wrp'}>
                      ...
                      <div className={'kq-check-label'}>
                        ...
                        <p className={'kq-check-submsg'}>
                          {' '}
                          <FormattedMessage
                            id='audit.kqlocation.label.submsg'
                            description='Audit location Csv upload error message'
                            defaultMessage='Selecting this will enable key in quantity for this audit task'
                          />
                        </p>
                ...
                <Tab
                  tabName={otherAuditMessg}
                  iconClassName={'icon-class-0'}
                  linkClassName={'link-class-0'}
                  internalTab={false}
                >
                  <div className='auditOtherContainer'>
                    <div className='audittypecontainer'>
                      <input
                        checked={true}
                        type='radio'
                        id='wallToWall'
                      ...
                      <label>
                        <FormattedMessage
                          id='createaudit.other'
                          description='Wall-to-Wall Audit'
                          defaultMessage='Wall-to-Wall Audit'
                        />
                      </label>
                    ...
                    <section className={'set-kq-wrp'}>
                      <div className={'kq-check-wrp'}>
                        <span>
                          <input
                            type='checkbox'
                            ref={input => {
                              this.kqCheckOtherLocation = input;
                            }}
                          />
                        </span>
                      </div>
                      <div className={'kq-check-label'}>
                        <p className={'kq-check-msg'}>
                          {' '}
                          <FormattedMessage
                            id='audit.kqotherlocation.label.msg'
                            description='Audit location Csv upload error message'
                            defaultMessage='Show KQ on Butler Operator Interface'
                          />
                        </p>
                        <p className={'kq-check-submsg'}>
                          {' '}
                          <FormattedMessage
                            id='audit.kqotherlocation.label.submsg'
                            description='Audit location Csv upload error message'
                            defaultMessage='Selecting this will enable key in quantity for this audit task'
                          />
                        </p>
            ...
            <div className={'audit-footer'}>
              <button
                ...
                className={
                  enableCreateAudit || this.state.activeTabIndex == 2
