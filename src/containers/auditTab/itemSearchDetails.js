      ...
      for (let i = 0, len = rawListData.length; i < len; i++) {
        ...
        for (let j = 0; j < datumProducts.length; j++) {
          ...
          if (datumState === 'complete') {
            ...
            if (productQuantity > 0) {
              itemStatusMessage = productQuantity + ' Excess item(s) found';
            } else {
              itemStatusMessage = 'No excess item found';
            }
          } else if (datumState === 'missing') {
            if (productQuantity > 0) {
              itemStatusMessage = productQuantity + ' item(s) missing';
            } else {
              itemStatusMessage = 'No missing item found';
            }
          }
        ...
        <div className='gor-modal-content pps-close'>
          <div className='gor-modal-head'>
            <div className='gor-usr-add'>
              {'Search item task - ' + this.props.itemSearchId}
            </div>
            <span className='close' onClick={this.props.removeModal}>
              ×
            </span>
          </div>
