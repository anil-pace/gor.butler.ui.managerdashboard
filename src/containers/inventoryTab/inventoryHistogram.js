...
class InventoryHistogram extends React.Component {
    ...
    _onClickCallBack(data) {
        this.props.selectData(data.customData);
    ...
    render() {
        ...
        return (
            <div>
                <Histogram hasDataChanged={this.props.hasDataChanged} config={config} histogramData={histogramData}
                           onClickCallBack={this._onClickCallBack.bind(this)}/>
            </div>
        )
    }
}
...
function mapDispatchToProps(dispatch) {
    ...
    return {
        setInventoryDate: function (data) {
            dispatch(setInventoryDate(data));
        }
    }
};
