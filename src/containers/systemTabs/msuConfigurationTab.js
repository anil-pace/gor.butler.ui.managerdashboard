...
class MsuConfigTab extends React.Component {
  ...
  manipulateButtonActions(arg) {
    ...
    if (arg.length > 0) {
      arg.forEach(eachMsu => {
        if (eachMsu.status === "reconfig_ready") {
          isAnyMsuEmpty.push(eachMsu.status)
        } else if (eachMsu.status === "waiting") {
          isAnyMsuDropping.push(eachMsu.status)
        } else if (eachMsu.status === "dropped") {
          isAnyMsuDropped.push(eachMsu.status)
        } else if (eachMsu.status === "ready_for_reconfiguration") {
          isAnyMsuReadyForReconfig.push(eachMsu.status)
        } else if (eachMsu.status === "storing_back") {
          isAnyMsuStoringBack.push(eachMsu.status)
        }
      })

      let checkAnyEmptyMsuFound = isAnyMsuEmpty.length > 0 ? true : false
      let checkAnyMsuInProgressFound =
        (isAnyMsuDropping.length > 0 ? true : false) ||
  ...
  _refreshList(query) {
    ...
    this.props.msuConfigFilterState({
      tokenSelected: {
        __typename: "MsuReconfigTokenSelected",
        STATUS: query.status
  ...
  render() {
    ...
    let noData = (
      <FormattedMessage
        id="msuConfig.table.noMsuData"
        description="Heading for no Msu Data"
        defaultMessage="No MSUs with blocked puts"
      />
    )
    return (
      <div>
          ...
          <div className="gorTesting gor-msu-config">
            ...
            {msuListData ? (
              <div>
                ...
                <div className="gorToolBar">
                  ...
                  <div className="filterWrapper">
                    ...
                    >
                      <div className="gor-button-wrap">
                        <button
                          disabled={this.state.releaseMsuBtnState}
                          className="gor-msuConfig-btn grey"
                          onClick={this._releaseMsuAction}
                        >
                          <FormattedMessage
                            id="gor.msuConfig.releaseMsu"
                    ...
                    >
                      <div className="gor-button-wrap">
                        <button
                          disabled={this.state.startStopBtnState}
                    ...
                    >
                      <div className="gor-button-wrap">
                        <button
                          ...
                          onClick={() => {
                            this.showMsuListFilter(true)
                          }}
                        >
                          <div className="gor-manage-task" />
                          <FormattedMessage
                            id="gor.msuConfig.filterLabel"
                ...
                {/*Filter Summary*/}
                <FilterSummary
                  total={msuListData.length || 0}
                  isFilterApplied={this.props.isFilterApplied}
                  responseFlag={this.props.responseFlag}
                  filterText={
                    <FormattedMessage
                      id="msuConfig.filter.search.bar"
                  ...
                  refreshText={
                    <FormattedMessage
                      id="msuConfig.filter.search.bar.showall"
            ...
            {msuListData.length === 0 ? (
              <div className="noDataWrapper"> {noData} </div>
            ) : (
              <MsuConfigTable
                items={msuListData}
                ...
                refreshList={this._clearFilter.bind(this)}
                blockPutAndChangeTypeCallback={this._blockPutAndChangeTypeCallback.bind(
                  this
                )}
              />
            )}
...
const withClientData = graphql(msuListClientData, {
  props: function(data) {
    return {
      showFilter: data.data.msuListFilter
        ? data.data.msuListFilter.display
        : false,
      isFilterApplied: data.data.msuListFilter
        ? data.data.msuListFilter.isFilterApplied
        : false,
      msuListFilterStatus: data.data.msuListFilter
        ? JSON.parse(JSON.stringify(data.data.msuListFilter.filterState))
        : null
    }
  }
})
