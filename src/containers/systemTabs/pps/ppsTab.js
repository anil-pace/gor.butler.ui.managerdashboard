...
class PPS extends React.Component {
  ...
  _processPPSData() {
    ...
    for (var i = data.length - 1; i >= 0; i--) {
      ...
      activeBins = data[i].active_bins
      totalBins = data[i].total_bins
      ppsBinDetails = nProps.context.intl.formatMessage(messages.ppsBinFooter, {
        currentPpsId: activeBins,
        totalPpsId: totalBins
      })
      detail.binDetails = ppsBinDetails

      detail.requested_status = requestedStatusText
      detail.pps_requested_mode = data[i]["requested_mode"]
      detail.isChecked = checkedPPS[data[i].pps_id] ? true : false
      if (data[i].status === PPS_STATUS_OPEN) {
        detail.status = OPEN
        detail.statusPriority = priStatus[data[i].status]
      } else if (data[i].status === PPS_STATUS_CLOSE) {
        detail.status = CLOSE
