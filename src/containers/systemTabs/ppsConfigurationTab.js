...
class PPSConfiguration extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
     ...
     */
    if (
      JSON.stringify(this.state.ppsList) !== JSON.stringify(nextProps.ppsList)
    ) {
  ...
  createNewPPSProfile(form_data) {
    ...
    return this.props.createPPSProfile(form_data).then(function(results, b, c) {
      return self.props.setPPSProfileCreated({
        profile: results.data.createPPSProfile
      })
    })
  }
      ...
      <div className="pps-configuration-container">
        ...
        {self.state.ppsList.length > 0 && (
          <PPSList
            ...
            selectPPSProfileForConfiguration={self.props.selectPPSProfileForConfiguration.bind(
              self
            )}
          />
        )}
        <div className="pps-configuration-details-container">
          <div className="pps-configuration-navigation-tabs">
            <div
              className={[
                "navigation-tab",
                this.state.currentView === "tags" ? "active" : ""
              ].join(" ")}
              onClick={this.handleClickOnNavigation.bind(this, "tags")}
            >
              <FormattedMessage
            ...
            <div
              className={[
                "navigation-tab",
                this.state.currentView === "bins" ? "active" : ""
              ].join(" ")}
              onClick={this.handleClickOnNavigation.bind(this, "bins")}
            >
              <FormattedMessage
                id="pps.configuration.bins.activation.label"
                description="Bin active/inactive"
                defaultMessage="Bin active/inactive"
              />
            </div>
            <div
              className={[
                "navigation-tab",
                this.state.currentView === "groups" ? "active" : ""
              ].join(" ")}
              onClick={this.handleClickOnNavigation.bind(this, "groups")}
            >
              <FormattedMessage
                id="pps.configuration.group.activation.label"
                description="Bin group active/inactive"
                defaultMessage="Bin group active/inactive"
              />
            </div>
            <div className={["seat-description"].join(" ")}>
              <FormattedMessage
          ...
          <Bins
            selectedPPSBinGroup={this.props.selectedPPSBinGroup}
            selectedPPSBin={this.props.selectedPPSBin}
            selectedPPS={this.props.selectedPPS}
            selectedProfile={this.props.selectedProfile}
            currentView={this.state.currentView}
          />{" "}
          {/* "currentView" will bes used to set the width of bins*/}
          {this.state.currentView === "tags" && (
            <Tags selectedPPSBin={this.props.selectedPPSBin} />
          )}
        </div>
        {this.props.selectedPPS && (
          <div className="pps-configuration-actions-container">
            <button
              onClick={self.cancelProfileChanges.bind(self)}
              className="pps-bin-cancel-button"
            >
              <FormattedMessage
            ...
            {this.state.currentView !== "groups" && (
              <button
                onClick={self.handleClickOnNext.bind(self)}
                className="pps-bin-next-button"
              >
                <FormattedMessage
            ...
            {this.state.currentView === "groups" && (
              <button
                disabled={
                  self.props.selectedPPS.pps_profiles.filter(function(profile) {
                    return profile.requested
                  }).length > 0 ||
                  self.props.selectedPPS.pps_status === PPS_STATUS_FCLOSE ||
                  self.props.selectedPPS.requested_status === PPS_STATUS_FCLOSE
                }
                onClick={self.saveAndApplyProfileConfirmation.bind(self)}
                className="pps-bin-save-apply-button"
              >
                <FormattedMessage
            ...
            {this.state.currentView !== "tags" && (
              <button
                onClick={self.handleClickOnBack.bind(self)}
                className="pps-bin-back-button"
              >
                <FormattedMessage
            ...
            {this.state.currentView === "groups" && (
              <button
                onClick={self.createProfile.bind(self)}
                className="pps-bin-save-button"
              >
                <FormattedMessage
            ...
            {this.state.currentView === "groups" && (
              <button
                disabled={self.props.selectedProfile.applied}
                onClick={self.saveProfile.bind(self, false)}
                className="pps-bin-save-button"
              >
                <FormattedMessage
 ...
 * @type {ComponentDecorator<TProps&TGraphQLVariables, TChildProps>}
 */
const withPPSProfileCreatedMutations = graphql(
  SET_PPS_PROFILE_CREATED_MUTATION,
  {
    props: ({ mutate, ownProps }) => ({
...
export default compose(
  ...
  createPPSProfileMutation
)(connect(mapStateToProps, mapDispatchToProps)(PPSConfiguration))
