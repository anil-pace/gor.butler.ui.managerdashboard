...
class PPSList extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (JSON.stringify(this.state.selectedPPS) !== JSON.stringify(nextProps.selectedPPS) || this.state.selectedProfile.profile_name !== nextProps.selectedProfile.profile_name) {
            ...
            <div>
                ...
                <div className="pps-list">
                    {this.state.data.map(function (pps) {
                        return <div
                            className={['pps-list-item', pps.pps_id === self.props.selectedPPS.pps_id ? 'selected' : null].join(" ")}
                            key={pps.pps_id} onClick={self.selectPPSProfile.bind(self, {pps: pps})}>
                            ...
                            <div className="pps-list-item-profiles">
                                {pps.pps_id === self.props.selectedPPS.pps_id && pps.pps_profiles.map(function (profile) {
                                    return <div className="pps-profile-item"
                                                onClick={self.selectPPSProfile.bind(self, {pps, profile})}
                                                key={profile.profile_name}>
                                        <div
                                            className={[profile.profile_name === self.props.selectedProfile.profile_name ? 'selected' : '', 'pps-profile-name'].join(" ")}>{profile.profile_name}</div>
                                        <div className="profile-label-container">
                                            {profile.applied &&
                                            <span className="profile-applied-label"><FormattedMessage
                                                id="pps.configuration.applied.text" description="Applied"
                                                defaultMessage="Applied"/></span>}
                                            {profile.requested && !profile.applied &&
                                            <span className="profile-requested-label"><FormattedMessage
                                                id="pps.configuration.requested.text" description="Requested"
                                ...
                                {pps.pps_id !== self.props.selectedPPS.pps_id && pps.pps_profiles.map(function (profile) {
                                    return profile.applied ?
                                        <div key={profile.profile_name}>{profile.profile_name} <FormattedMessage
