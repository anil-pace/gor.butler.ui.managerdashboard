...
class UtilityTab extends React.Component {
        ...
        >
          <div>
            <input
              ...
              onChange={this._handleChangeStockLedgerCategory.bind(this)}
              type="radio"
              id="all"
              name="stock_ledger"
              value="all"
      ...
      <div key="1">
        <div className="gor-audit-input-wrap gor-utility-sku-wrap">
          <input
            ...
            disabled={this.state.stockLedgerState.category === "all"}
      ...
      <div key="2" style={{ marginBottom: 10, fontSize: 13 }}>
        ...
        <div className="gor-utility-duration-wrap">
          <div style={{ width: "50%", float: "left" }}>
            <input
              onChange={this._handleChangeStockLedgerDuration.bind(this)}
  ...
  _onMasterFileUpload(fileObject) {
    ...
    this.props
      .uploadMasterData({ file: fileObject })
      .then(() => {
        this._onMDMRefresh()
      })
      .finally(() => this.setState({ isMasterUploadProcessing: false }))
  }
  ...
  _downloadStockLedger() {
    ...
    params.push(
      [
        "yesterday",
        this.state.stockLedgerState.duration.yesterday ? "true" : "false"
      ].join("=")
    )
  ...
  _onMDMRefresh() {
    this.props.client
      ...
      .then(data =>
        this.setState({
          uploadHistoryData: data.data.MasterDataUploadHistory.list || []
        })
      )
  }
  ...
  render() {
    ...
    try {
      show_item_recall_scripts = this.props.config.utility_tab.widgets.scripts
        .item_recall
    } catch (ex) {
      //Do nothing
    }
    ...
    try {
      show_stock_ledger_widget = this.props.config.utility_tab.widgets.reports
        .stock_ledger_report
    } catch (ex) {
      //Do nothing
    }
    ...
    try {
      show_stock_ledger_raw_transaction_widget = this.props.config.utility_tab
        .widgets.reports.stock_ledger_raw_transactions_report
    } catch (ex) {
      //Do nothing
    }
    ...
    return (
      <div>
        {show_item_recall_scripts ? (
          <UtilityTile
            tileHead={this.context.intl.formatMessage(messages.itemsRecall)}
            showFooter={false}
        ...
        {show_masterdata_upload ? (
          <UtilityTile
            tileHead={this.context.intl.formatMessage(messages.masterDataHead)}
            showFooter={false}
            showHeaderIcon={true}
            onRefresh={this._onMDMRefresh.bind(this)}
          >
            <MasterUploadTile
              uploadHistChanged={this.props.uploadHistChanged}
              uploadBtnText={this.context.intl.formatMessage(
                messages.uploadBtnText
              )}
              isMasterUploadProcessing={this.state.isMasterUploadProcessing}
              ...
              historyData={this.state.uploadHistoryData || []}
        ...
        {show_stock_ledger_widget ? (
          <UtilityTile
            ...
            tileHead={this.context.intl.formatMessage(messages.stockLedgerHead)}
            showFooter={true}
            footerAction={this._downloadStockLedger.bind(this)}
        ...
        {show_stock_ledger_raw_transaction_widget ? (
          <UtilityTile
            loading={this.props.stockLedgerRawTransactionsSpinner}
            tileHead={this.context.intl.formatMessage(
              messages.stockLedgerRawTransactionHead
            )}
            showFooter={true}
            footerAction={this._downloadStockLedgerRawTransactions.bind(this)}
            enableButton={true}
          >
...
export default compose(withApollo)(
  connect(
    mapStateToProps,
    mapDispatchToProps
  )(compose(withMasterDataUpload)(UtilityTab))
)
