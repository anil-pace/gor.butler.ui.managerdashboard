...
class ResumeOperation extends React.Component {
  ...
  _typing() {
    this.passField.className =
      'gor-password-field-lg gor-input-ok gor-input-typing'
    this._checkPass()
  }
  _checkPass() {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      !nextProps.breached &&
      (!nextProps.auth_token ||
        !nextProps.system_emergency ||
        nextProps.system_data !== this.props.system_data)
    ) {
      if (
        nextProps.system_emergency ||
        nextProps.fireHazard.emergency_type !== EMERGENCY_FIRE
      )
        this._removeThisModal()
    }
    if (nextProps.modalStatus && !this.props.modalStatus) {
      let typeFlag = this.props.fireHazardPressed
        ? EMERGENCY_FIRE
        : SYSTEM_EMERGENCY
      this._removeThisModal()
        ...
        <div className='gor-operation-body'>
          ...
          <div className='gor-margin-top'>
            ...
            <button
              ...
              disabled={
                this.props.passwordCheck.type === SUCCESS ? false : true
              }
              onClick={this._handleResume.bind(this)}
            >
              <FormattedMessage
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    modalresetForm: function() {
      dispatch(modalFormReset())
    },
    resetPasswordButton: function() {
      dispatch(resetPasswordButton())
    }
  }
}
ResumeOperation.propTypes = {
  ...
  resetPasswordButton: React.PropTypes.func
}
