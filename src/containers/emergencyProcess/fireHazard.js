...
import { humanize } from "../../utilities/humanizeString"
class FireHazard extends React.Component {
  ...
  shouldComponentUpdate() {
    if (!this.props.fireHazard.sourceOfFire) this._removeThisModal()
    return true
  }
  componentDidUpdate() {
    if (!this.props.auth_token || !this.props.fireHazard.sourceOfFire) {
  ...
  processShutterHeader(data) {
    ...
    if (p > 0) {
      marker = (
        <div>
          <span className="gor-image-status progress"></span>{" "}
      ...
      marker = (
        <div>
          <span className="gor-image-status failed"></span>{" "}
          <span className="gor-status-text">
            {" "}
            <FormattedMessage
              id="operation.fire.failshutter"
              defaultMessage="Failed to clear shutters"
              description="Description Text fail to clear shutter"
            />{" "}
          </span>
        </div>
      )
      ...
      marker = (
        <div>
          <span className="gor-image-status cleared"></span>{" "}
          <span className="gor-status-text">
            {" "}
            <FormattedMessage
              id="operation.fire.clearedshutter"
              defaultMessage="Cleared all shutters"
              description="Description Text  cleared shutter"
            />{" "}
          </span>
        </div>
      )
    ...
    if (data === CLEARED) {
      escapeMarker = (
        <div>
          ...
          <span className="gor-status-text">
            <FormattedMessage
              id="operation.fire.escapecleared"
              defaultMessage="Cleared escape path"
              description="Description Text escape cleared"
            />
          </span>
        </div>
      )
      ...
      escapeMarker = (
        <div>
          ...
          <span className="gor-status-text">
            <FormattedMessage
              id="operation.fire.escapeprogress"
              defaultMessage="Clearing escape path"
              description="Description Text escape in progress"
            />
          </span>
        </div>
      )
      ...
      escapeMarker = (
        <div>
          ...
          <span className="gor-status-text">
            <FormattedMessage
              id="operation.fire.escapefail"
              defaultMessage="Failed to clear escape path"
              description="Description Text escape to fail"
            />
          </span>
        </div>
      )
  ...
  render() {
    var shutterFlag = false,
      escapePathFlag = false
    var shutterNuber = Object.keys(this.props.fireHazard.shutters).length
    shutterFlag = shutterNuber === 0
    escapePathFlag =
      this.props.fireHazard.escapePath !== NOT_FOUND ? true : false
    var shutterWrap = this._processData()
    var markerShutter = this.processShutterHeader(
      this.props.fireHazard.shutters
    )
    var markerEscapePath = this.processEscapePath(
      this.props.fireHazard.escapePath
    )
    var isFireTriggered = this.props.fireHazard.sourceOfFire
      ? humanize(this.props.fireHazard.sourceOfFire)
      : null
    var messageToShow = (
      <span>
        {" "}
        Fire emergency has been triggered by <b>{isFireTriggered}</b> and system
        has started its safety measures.
      </span>
    )
    return (
      <div className="gor-modal-content gor-firehazard">
        <span className="hazard-image"></span>
        <span className="close" onClick={this._removeThisModal}>
          ×
        </span>
        ...
        <div
          className={
            !shutterFlag ? "gor-shutter-section" : "gor-shutter-section hidden"
          }
        >
          {markerShutter}
          <div className="gor-shutter-status-box">{shutterWrap}</div>
        </div>
        <button
          className={
            this.state.buttonDisable
              ? "gor-resume-btn disable"
              : "gor-resume-btn"
          }
          disabled={this.state.buttonDisable}
          onClick={this._resumeOperation}
        >
          <FormattedMessage
