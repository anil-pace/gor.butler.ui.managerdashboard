  ...
  return store => next => action => {
    switch (action.type) {
      case AJAX_CALL:
        ...
        var saltParams = action.params.saltParams
          ? action.params.saltParams
          : {};
        ...
        httpRequest.onreadystatechange = function(xhr) {
          if (httpRequest.readyState === XMLHttpRequest.DONE) {
            if (
              ...
              authCause.indexOf(params.cause) == -1
            ) {
              endSession(store);
            } else {
              try {
                let resContentType = httpRequest.getResponseHeader(
                  'Content-type'
                );
                if (
                  resContentType &&
                  (resContentType.match(/(text\/csv)/g) ||
                    resContentType.match(
                      /(application\/vnd.openxmlformats-officedocument.spreadsheetml.sheet)/g
                    ) ||
                  ...
                  if (this.getResponseHeader('Content-disposition')) {
                    let strName = this.getResponseHeader(
                      'Content-disposition'
                    ).match(/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/);
                    fileName = strName[0].slice(10, strName[0].length - 1);
                  }
                  fileName = !fileName
                    ? resContentType.match(/(text\/csv)/g)
                      ? 'download.csv'
                  ...
                  let decodedString =
                    httpRequest.responseType === 'arraybuffer'
                      ...
                      : httpRequest.response;
                  var response = decodedString
                    ? JSON.parse(decodedString, httpRequest.status)
                    : decodedString;
                  AjaxParse(
                    store,
                    response,
