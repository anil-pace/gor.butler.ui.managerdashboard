...
import {
  wsOrdersHeaderUnSubscribe,
  wsOrdersHeaderSubscribe
...
class App extends React.Component {
  ...
  componentWillMount() {
    if (this.props.location.pathname !== "/") {
      sessionStorage.setItem(
        "nextUrl",
        this.props.location.pathname.concat(this.props.location.search)
      );
    }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let loginAuthorized = nextProps.loginAuthorized,
      ...
      currTab = nextProps.location.pathname.substring(
        1,
    ...
    if (currTab) {
      subscribeData =
        nextProps.wsSubscriptionData[currTab] ||
        nextProps.wsSubscriptionData["default"];
    } else {
    ...
    if (
      !this.state.platformSocketConnected &&
      nextProps.notificationSocketConnected
    ) {
      this.setState(
        {
...
function mapStateToProps(state, ownProps) {
  return {
    ...
    notificationSocketConnected:
      state.notificationSocketReducer.notificationSocketConnected
  };
}
/**
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    initMockData: function(data) {
      dispatch(setMockAction({ type: WS_MOCK, data: data }));
    },
    prevTabSelected: function(data) {
      dispatch(prevTabSelected(data));
    },
    getTimeOffSetData: function(data) {
      dispatch(getTimeOffSetData(data));
    },
    setTimeOffSetData: function(data) {
      dispatch(setTimeOffSetData(data));
    },
    endConnect: function() {
      dispatch(endWsAction());
    },
    userLogout: function() {
      dispatch(logoutRequest());
    },
    notifyInfo: function(data) {
