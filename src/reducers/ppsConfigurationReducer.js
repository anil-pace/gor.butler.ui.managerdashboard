...
import {
  ...
  RECEIVE_TAGS,
  CANCEL_PROFILE_CHANGES,
  CHANGE_PPS_BIN_GROUP_STATUS,
  SELECT_PPS_BIN_GROUP,
  PPS_PROFILE_CREATED,
  PPS_PROFILE_SAVED,
  TAG_ADDED_TO_LIST,
  DISPLAY_PPS_CONFIGURATION_SPINNER,
  PPS_PROFILE_REQUESTED
  ...
  switch (action.type) {
    ...
    case RECEIVE_PPS_PROFILES:
      ...
      if (state.selectedPPS && state.selectedPPS.pps_id) {
        selected_pps = JSON.parse(
          JSON.stringify(
            pps_list.filter(function(pps) {
              return pps.pps_id === state.selectedPPS.pps_id
            })[0]
          )
        )
      } else {
    ...
    case SELECT_PPS_PROFILE_FOR_CONFIGURATION:
       ...
       */
      selected_pps = JSON.parse(
        JSON.stringify(action.data.pps || state.selectedPPS)
      )
      ...
      if (action.data.pps_profiles) {
         ...
         * and selects it.
         */
        selected_profile = action.data.pps_profiles
          ? JSON.parse(
              JSON.stringify(
                action.data.pps_profiles.filter(function(profile) {
                  return profile.bin_details
                })[0]
              )
            )
          : null
        selected_pps.pps_profiles = JSON.parse(
          JSON.stringify(action.data.pps_profiles)
        )
        pps_list = state.ppsList.map(function(pps) {
          if (pps.pps_id === selected_pps.pps_id) {
            pps.pps_profiles = JSON.parse(
              JSON.stringify(action.data.pps_profiles)
            )
          }
          return pps
        ...
        selected_profile = JSON.parse(
          JSON.stringify(
            selected_pps.pps_profiles.filter(function(profile) {
              return profile.bin_details
            })[0]
          )
        )
    ...
    case SELECT_PPS_BIN:
      ...
      selected_bin.id = [
        state.selectedPPS.pps_id,
        selected_bin.pps_bin_id
      ].join("-")
      return Object.assign({}, state, {
    ...
    case CHANGE_PPS_BIN_GROUP_STATUS:
      ...
      selected_profile.bin_group_details = selected_profile.bin_group_details.map(
        function(group) {
    ...
    case PPS_PROFILE_SAVED:
      ...
      selected_pps.pps_profiles = selected_pps.pps_profiles.map(function(
        profile
      ) {
        if (profile.profile_name === selected_profile.profile_name) {
          profile = selected_profile
