...
function processButlersData(data) {
  let butlerData = [],
    butlerDetail = {};
  const online = (<FormattedMessage
    id="butlerDetail.online.status"
    description="online status for butlerDetail"
    defaultMessage="Online"
  />);
  const offline = (<FormattedMessage
    id="butlerDetail.offline.status"
    description="offline status for butlerDetail"
    defaultMessage="Offline"
  />);
  const currentTask = { 0: 'Pick', 1: 'Put', 2: 'Audit', 3: 'Charging', 4: 'Move' };
  const currentSubtask = { 0: 'Moving to', 1: 'Moving to mount', 2: 'Moving to dismount', 3: 'Docked at' };
  const currentState = { online, offline };
  let BOT,
    PPS,
    CS,
    MSU;
